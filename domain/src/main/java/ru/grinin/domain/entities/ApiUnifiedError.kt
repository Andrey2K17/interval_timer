...
sealed class ApiUnifiedError(open val message: String?, open val code: Int?) {
    ...
    sealed class Http(message: String?, errorCode: Int) : ApiUnifiedError(message, errorCode) {
        data class Unauthorized(override val message: String?, val errorCode: Int) :
            Http(message, errorCode)

        data class NotFound(override val message: String?, val errorCode: Int) :
            Http(message, errorCode)

        data class InternalError(override val message: String?, val errorCode: Int) :
            Http(message, errorCode)

        data class BadRequest(override val message: String?, val errorCode: Int) :
            Http(message, errorCode)
    }
